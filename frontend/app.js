...
const filledFormOverlay = document.getElementById("filledFormOverlay");
const closeReviewBtn = document.getElementById("closeReviewBtn");
const filledFormContent = document.getElementById("filledFormContent");
...
let currentDocResult = null; // Store the full doc result for review

// --- Form Storage Logic ---
function getFieldStorageKey(docId, fieldName) {
  return `nagrik_form_${docId}_${fieldName.toLowerCase().replace(/\s+/g, '_')}`;
}

function saveFormField(docId, fieldName, value) {
  if (!docId) return;
  localStorage.setItem(getFieldStorageKey(docId, fieldName), value);
}

function getFormField(docId, fieldName) {
  if (!docId) return "";
  return localStorage.getItem(getFieldStorageKey(docId, fieldName)) || "";
}
...
const I18N = {
  en: {
    ...
    logo_subtext: "Making Govt Forms Easy",
    bento_title: "Confused?",
    bento_sub: "Tap 'Listen' above or Ask AI",
    review_btn: "Review & See Form",
    summary_modal_title: "Your Filled Form (Exact Copy)",
    download_btn: "Download (Exact PDF)",
    download_sub: "This PDF will look exactly like your original form.",
    page_tab_label: "Page",
    what_to_fill: "What to fill",
    btn_listen: "<span class=\"icon\">🔊</span> Listen",
    aadhaar_warning: "<span>⚠️</span> <strong>Note:</strong> Aadhaar number must be exactly 12 digits. No spaces or dashes."
  },
  hi: {
    ...
    example: "उदाहरण",
    btn_listen: "<span class=\"icon\">🔊</span> सुनें",
    btn_listening: "⏳ आवाज़...",
    ...
    // Ask AI
    ask_ai_btn: "पूछें AI से",
    ask_ai_title: "AI डैशबोर्ड",
    ...
    logo_subtext: "सरकारी फ़ॉर्म अब आसान",
    bento_title: "समझ न आए?",
    bento_sub: "ऊपर दिए गए 'सुनें' बटन दबाएं या पूछें",
    review_btn: "रिव्यू और फॉर्म देखें",
    summary_modal_title: "आपका भरा हुआ फॉर्म (Exact Copy)",
    download_btn: "डाउनलोड करें (Exact PDF)",
    download_sub: "यह पीडीएफ आपके ऑरिजिनल फॉर्म जैसा ही दिखेगा।",
    page_tab_label: "पेज",
    aadhaar_warning: "<span>⚠️</span> <strong>ध्यान दें:</strong> आधार नंबर 12 अंकों का ही होना चाहिए। स्पेस या डैश न लगाएं।"
  }
};
...
function applyLanguage(lang) {
  ...
  const locMap = {
    ...
    "resFooterBackText": t.btn_footer_back,
    "resFooterNextText": t.btn_footer_next,
    "resFooterReplayText": t.btn_footer_replay,
    ...
    "logo-subtext-el": t.logo_subtext,
    "ask-ai-bento-title-el": t.bento_title,
    "ask-ai-bento-sub-el": t.bento_sub,
    "askAiCtaBtnText": t.ask_ai_btn,
    "reviewFormBtnText": t.review_btn,
    "summary-title-modal-el": t.summary_modal_title,
    "downloadBtnText": t.download_btn,
    "downloadSubtext": t.download_sub
  };
...
function renderResult(data) {
  ...
  data.pages.forEach((p, idx) => {
    ...
    btn.textContent = `${t.page_tab_label} ${idx + 1}`;
    btn.onclick = () => {
...
function renderFields(fields, pageNum, pageData = null) {
  ...
  fields.forEach((f, idx) => {
    ...
    const isRequired = (f.required !== undefined) ? f.required : true;
    const inputType = f.input_type || "text";
    const docId = resultsSection.dataset.currentId;
    const userValue = getFormField(docId, fieldName);
    ...
    row.innerHTML = `
      ...
      <div class="field-col-data">
        <div class="field-title-row">
          <h3>${fieldName}</h3>
          ${isRequired ? `<span class="tag-required">${t.important || "महत्वपूर्ण"}</span>` : ""}
        </div>
        <div class="field-instruction-line" style="margin-bottom: 8px;">
          <strong style="color: #2563eb;">${t.what_to_fill}:</strong> ${whatToFill}
        </div>
        
        <!-- Interactive Input Field with Voice option -->
        ${(fieldName.toLowerCase().includes("signature") || fieldName.includes("हस्ताक्षर")) ? `
          <div class="signature-notice" style="margin-top: 12px; padding: 12px; background: #fef2f2; border: 1px dashed #fecaca; border-radius: 8px; font-size: 14px; color: #991b1b; font-weight: 600;">
            🖋️ ${lang === 'hi' ? 'कृपया यहाँ शारीरिक रूप से हस्ताक्षर करें।' : 'Please sign here physically on the document.'}
          </div>
        ` : `
          <div class="field-input-box" style="margin-top: 12px; position: relative; display: flex; gap: 8px;">
            <input type="${inputType}" 
                   class="nagrik-form-input" 
                   placeholder="${lang === 'hi' ? 'यहाँ भरें...' : 'Type here...'}" 
                   data-field="${fieldName}"
                   value="${userValue}"
                   style="flex: 1; padding: 10px 14px; border-radius: 8px; border: 2px solid #e2e8f0; font-family: inherit;">
            <button class="btn-mic-field" 
                    title="Speak this field"
                    style="width: 44px; height: 44px; border-radius: 50%; background: #f1f5f9; border: 1px solid #e2e8f0; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s; padding: 0;">
          ...
          <div class="validation-msg" style="color: #e11d48; background: #fff1f2; padding: 8px 12px; border-radius: 8px; margin-top: 8px; font-size: 13px; font-weight: 600; display: none; transition: all 0.2s;"></div>
        `}
      </div>
      
      <div class="field-col-action">
        <button class="btn-audio-mini" data-text="${whatToFill}." style="background: #2563eb; color: white; border: none; border-radius: 8px; padding: 10px 16px; font-weight: 600;">
          ${t.btn_listen}
        </button>
      </div>
    `;
    ...
    if (fieldName.includes("आधार") || fieldName.toLowerCase().includes("aadhaar")) {
      const warningBar = document.createElement("div");
      warningBar.className = "note-bar";
      warningBar.innerHTML = t.aadhaar_warning;
    ...
    // Handle manual input and field-level voice input
    const input = row.querySelector(".nagrik-form-input");
    const statusHint = row.querySelector(".input-status-hint");
    const micFieldBtn = row.querySelector(".btn-mic-field");
    const valMsg = row.querySelector(".validation-msg");

    if (input) {
      input.oninput = (e) => {
        const val = e.target.value;
        saveFormField(docId, fieldName, val);
        if (statusHint) statusHint.style.display = val ? "block" : "none";
        
        let error = "";
        const nameL = fieldName.toLowerCase();
        const cleanVal = val.replace(/\s/g, ''); // ignore spaces for numeric check
        
        // Auto-validation rules
        if (cleanVal.length > 0) {
          if ((nameL.includes("आधार") || nameL.includes("aadhaar") || nameL.includes("adhar")) && !/^\d{12}$/.test(cleanVal)) {
            error = lang === "hi" ? "आधार नंबर 12 अंकों का होना चाहिए।" : "Aadhaar must be exactly 12 digits.";
          } else if ((nameL.includes("मोबाइल") || nameL.includes("phone") || nameL.includes("mobile")) && !/^\d{10}$/.test(cleanVal)) {
            error = lang === "hi" ? "मोबाइल नंबर 10 अंकों का होना चाहिए।" : "Mobile number must be exactly 10 digits.";
          } else if ((nameL.includes("पिन") || nameL.includes("pincode") || nameL.match(/\bpin\b/) || nameL.includes("zip")) && !/^\d{6}$/.test(cleanVal)) {
            error = lang === "hi" ? "पिन कोड 6 अंकों का होना चाहिए।" : "PIN code must be exactly 6 digits.";
          } else if ((nameL.includes("पैन") || nameL.includes("pan card") || nameL.match(/\bpan\b/)) && !/^[A-Za-z0-9]{10}$/.test(cleanVal)) {
            error = lang === "hi" ? "पैन कार्ड 10 अक्षरों का होना चाहिए।" : "PAN card must be exactly 10 characters.";
          } else if (nameL.includes("ifsc") && !/^[A-Za-z0-9]{11}$/.test(cleanVal)) {
            error = lang === "hi" ? "IFSC कोड 11 अक्षरों का होना चाहिए।" : "IFSC code must be exactly 11 characters.";
          } else if ((nameL.includes("खाता") || nameL.includes("account")) && !/^\d{9,18}$/.test(cleanVal)) {
            error = lang === "hi" ? "खाता संख्या 9 से 18 अंकों के बीच होनी चाहिए।" : "Account number must be between 9 and 18 digits.";
          } else if ((nameL.includes("ईमेल") || nameL.includes("email")) && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(cleanVal)) {
            error = lang === "hi" ? "कृपया सही ईमेल दर्ज करें (जैसे: name@gmail.com)।" : "Please enter a valid email format.";
          } else if ((nameL.includes("वर्ष") || nameL.includes("साल") || nameL.includes("year")) && !/^\d{4}$/.test(cleanVal)) {
            error = lang === "hi" ? "वर्ष 4 अंकों का होना चाहिए (जैसे: 2024)।" : "Year must be exactly 4 digits (e.g. 2024).";
          } else if ((nameL.includes("उम्र") || nameL.includes("आयु") || nameL.match(/\bage\b/)) && (!/^\d{1,3}$/.test(cleanVal) || parseInt(cleanVal) > 120 || parseInt(cleanVal) <= 0)) {
            error = lang === "hi" ? "कृपया सही उम्र दर्ज करें (1 से 120)।" : "Please enter a valid age (1-120).";
          } else if ((nameL.includes("राशि") || nameL.includes("amount") || nameL.includes("rupees") || nameL.includes("रुपये")) && !/^\d+(\.\d{1,2})?$/.test(cleanVal)) {
            error = lang === "hi" ? "कृपया सही राशि दर्ज करें (केवल नंबर)।" : "Please enter a valid amount (numbers only).";
          }
        }
    ...
    if (micFieldBtn) {
      ...
      micFieldBtn.style.background = "#fee2e2"; // Pulsing red
      micFieldBtn.style.borderColor = "#ef4444";
      
      recognition.onresult = (event) => {
        const transcript = event.results[0][0].transcript;
        input.value = transcript;
        saveFormField(docId, fieldName, transcript);
        statusHint.style.display = "block";
      };
      
      recognition.onend = () => {
        micFieldBtn.style.background = "#f1f5f9";
        micFieldBtn.style.borderColor = "#e2e8f0";
      };
...
// Voice-First Interaction: Only Mic listener remains
// Keyboard and Send button listeners removed per user request
...
// Interface of select language Hindi English while entering the application
...
// --- Document Reconstruction Logic (EXACT REPLICA) ---
function reconstructDocument(currentDoc) {
  const docId = resultsSection.dataset.currentId;
  let finalHtml = "";

  currentDoc.pages.forEach((page, idx) => {
    let md = page.original_markdown || "";
    
    // Process fields on this page
    if (page.fields) {
      page.fields.forEach(f => {
        const label = f.field_name || f.label;
        const val = getFormField(docId, label);
        if (!val) return;

        // Escape label for regex
        const escapedLabel = label.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        
        // 1. Exact Match Strategy: find label + some optional text + blanks/underscores
        const regex = new RegExp(`(${escapedLabel}[^_\.\\n]*?)([_\\.]{2,}|\\s{3,}|$)`, 'gm');
        
        let found = false;
        md = md.replace(regex, (match, p1, p2) => {
          found = true;
          return `${p1} <span class="filled-ink">${val}</span>`;
        });
        
        // 2. Fallback strategy if blanks don't exist
        if (!found) {
           const fallbackRegex = new RegExp(`(${escapedLabel}[^\\n]*)`, 'm');
           if (fallbackRegex.test(md)) {
               md = md.replace(fallbackRegex, `$1 <span class="filled-ink">${val}</span>`);
           } else {
               md += `\n<br><strong>${label}:</strong> <span class="filled-ink">${val}</span>`;
           }
        }
      });
    }

    // Convert markdown to presentable HTML (Simple headers/bold only for exact reproduction)
    let pageHtml = md
      .replace(/^# (.*$)/gim, '<h1 style="text-align: center;">$1</h1>')
      .replace(/^## (.*$)/gim, '<h2>$1</h2>')
      .replace(/^### (.*$)/gim, '<h3>$1</h3>')
    ...
    finalHtml += `<div class="reconstructed-page-content">${pageHtml}</div>`;
    if (idx < currentDoc.pages.length - 1) {
      finalHtml += `<div class="form-page-divider"></div>`;
    }
  });

  return finalHtml || "--- No content available ---";
}

// --- Final Form Review Logic ---
function showFinalReview() {
  const docId = resultsSection.dataset.currentId;
  const history = Storage.getAll();
  const currentDoc = history.find(d => d.id === docId);
  
  if (!currentDoc || !currentDoc.pages) return;
  
  filledFormContent.innerHTML = reconstructDocument(currentDoc);
  filledFormOverlay.hidden = false;
  filledFormOverlay.style.display = "flex";
}

if (reviewFormBtn) reviewFormBtn.onclick = showFinalReview;
if (closeReviewBtn) closeReviewBtn.onclick = () => {
    filledFormOverlay.hidden = true;
    filledFormOverlay.style.display = "none";
};

// --- PDF Export with html2pdf.js ---
if (downloadFormBtn) {
  downloadFormBtn.onclick = () => {
    const docId = resultsSection.dataset.currentId;
    const history = Storage.getAll();
    const currentDoc = history.find(d => d.id === docId);
    ...
    const opt = {
      margin:       10,
      filename:     `Filled_Form_${(currentDoc.name || "Replica").replace(/\s+/g, '_')}.pdf`,
    ...
    // Show loading state
    const originalText = downloadFormBtn.innerHTML;
    downloadFormBtn.innerHTML = "⏳ Generating Official PDF...";
    downloadFormBtn.disabled = true;

    html2pdf().set(opt).from(element).save().then(() => {
      downloadFormBtn.innerHTML = originalText;
      downloadFormBtn.disabled = false;
    }).catch(err => {
      console.error("PDF Export Error:", err);
      downloadFormBtn.innerHTML = "❌ Export failed. Try again.";
      downloadFormBtn.disabled = false;
    });
  };
}
