...
NOISE_LABELS = {
    ...
    "instructions", "instruction", "naac",
    "yes", "no", "ha", "nahi", "हाँ", "नहीं",
    # Keeping only very technical noise, allowing headers like 'Form', 'Ministry', 'Date'
    "circular", "affiliated to", "approved by",
...
def _append_field(
    ...
    if not _looks_like_label(clean):
        return
    fields.append(
        {
            "label": clean,
    ...
    while i < len(lines):
        ...
        if (
            ...
            for cell in header_cells:
                _append_field(fields, cell, "table_header", line_no, "")
            ...
            while i < len(lines):
                ...
                for cell in row_cells:
                    _append_field(fields, cell, "table_cell", i + 1, "")
                i += 1
            continue
        ...
        if len(frags) > 1:
            for m in frags:
                _append_field(fields, m.group(1), "label_value_multi", line_no, m.group(2).strip())
            i += 1
            continue
        ...
        if m:
            _append_field(fields, m.group(1), "label_blank", line_no, "")
        ...
        if not frags:
            ...
            if m:
                _append_field(fields, m.group(1), "label_value", line_no, m.group(2).strip())
                i += 1
                continue
        ...
        if m:
            _append_field(fields, m.group(1), "trailing_blank", line_no, "")
            i += 1
            continue
        ...
        if i + 1 < len(lines):
            ...
            if next_is_blank_or_fill:
                ...
                if m:
                    _append_field(fields, m.group(1), "standalone_label", line_no, "")
